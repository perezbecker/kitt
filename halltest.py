...
GPIO.setup(33, GPIO.IN)    # set GPIO13 as input  
  
# Define a threaded callback function to run in another thread when events are detected  
def my_callback(channel):  
    if GPIO.input(33):     # if port 25 == 1  
        print "Rising edge detected on 33"  
    else:                  # if port 25 != 1  
...
print "GPIO port 13 (pin 33) to GND (pin 6) through a ~1k resistor\n"  
...
try:  
    print "When pressed, you'll see: Rising Edge detected on 33"  
    print "When released, you'll see: Falling Edge detected on 33"  
