...
GPIO.setup(29, GPIO.IN)    # set GPIO13 as input  
  
# Define a threaded callback function to run in another thread when events are detected  
def my_callback(channel):  
    if GPIO.input(29):     # if port 25 == 1  
        print "Rising edge detected on 29"  
    else:                  # if port 25 != 1  
...
print "GPIO port 05 (pin 29) to GND (pin 6) through a ~1k resistor\n"  
...
try:  
    print "When pressed, you'll see: Rising Edge detected on 29"  
    print "When released, you'll see: Falling Edge detected on 29"  
